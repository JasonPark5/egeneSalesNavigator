...
// BACKEND_MODE=actionflow일 때 'search'(text/chip/locate) 요청을 처리한다.
//
// 예전엔 이 전체(즐겨찾기 매칭 + 원점/지명 해석 + 카카오 폴백 4단계 + LLM 의도분석/큐레이션)를
// ActionFlow 캔버스 하나에 노드 30여 개로 옮기려고 했는데, Condition이 단일 비교만 가능하고
// Agent가 tool use를 지원하지 않는 등 제약 때문에 유지보수가 어려운 초대형 플로우가 됐다.
//
// 대신: 분기/카카오 검색 로직은 pipeline.js의 runSearchPipeline()을 그대로 재사용한다
// (mock 모드와 100% 동일한 코드 — 두 모드가 서로 다르게 동작할 여지가 없다). ActionFlow는
// 정말 LLM이 필요한 두 지점, 딱 그만큼만 담당한다:
...
// 이유가 없다. 그래서 chip/locate(LLM을 아예 안 씀)는 ActionFlow를 전혀 호출하지 않는다.
//
